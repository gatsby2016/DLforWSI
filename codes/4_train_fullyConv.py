...
net = resnet34(pretrained=False, num_classes=num_class).cuda()  # num_class is 10
# net.load_state_dict(torch.load('../modelFullyConv/epoch_0.pkl')) # load the finetune weight parameters
#
...
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, [2, 20], gamma=0.3)
...
for epoch in range(epoches):
    ...
    with torch.no_grad():
        for i, (img, target) in tqdm(enumerate(testloader)):
            ...
            prediction = torch.argmax(F.softmax(net(img.cuda()).squeeze(3).squeeze(2), 1, _stacklevel=5), dim=1)
    ...
    torch.save(net.state_dict(), '../modelFullyConv/epoch_'+str(epoch)+'.pkl')
